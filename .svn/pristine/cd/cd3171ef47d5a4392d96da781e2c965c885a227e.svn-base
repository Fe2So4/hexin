  ...
  <div id="rate-index" >
    <ui-notice-bar slot="breadcast" >
      <el-breadcrumb slot="location" separator=">">
        ...
        <el-breadcrumb-item>内蒙古寿险折标系数管理</el-breadcrumb-item>
      </el-breadcrumb>
    </ui-notice-bar>
        ...
        <div class="ri-content" slot="content" >
            ...
            <el-col :span="12">
              <!-- <el-form-item class="insurerCode" prop="insurerCode"> -->
                <ui-label-text labelWidth="120" label="保险公司代码:" >
                  <el-input v-model="info.insurerCode" slot="text" placeholder="双击可选择" clearable  @dblclick.native="dbqueryCom" @blur="insuerBlur" ></el-input>
                </ui-label-text>
              <!-- </el-form-item> -->
            </el-col>
            <el-col :span="12">
              ...
              <ui-label-text labelWidth="120" label="保险公司名称:" >
                <el-input v-model="info.insurerName" slot="text" placeholder="双击可选择" clearable @dblclick.native="dbqueryCom" @blur="insuerBlur"></el-input>
              </ui-label-text>
              <!-- </el-form-item> -->
            </el-col>
          </el-row>
          <el-row class="ri-line">
            ...
            <ui-label-text labelWidth="120" label="险种代码:" >
              <el-input v-model="info.riskCode" slot="text" placeholder="双击可选择" clearable @dblclick.native="dbqueryRisk"  @blur="riskBlur"></el-input>
            </ui-label-text>
            </el-col>
            <el-col :span="12">
            <ui-label-text labelWidth="120" label=" 险种名称 :" >
              <el-input v-model="info.riskName" slot="text" placeholder="双击可选择" clearable @dblclick.native="dbqueryRisk"  @blur="riskBlur"></el-input>
            </ui-label-text>
            </el-col>
          </el-row>
          </el-form>
    ...
    <!-- 双击弹框 -->
    <el-dialog :append-to-body="true" width="55%" title="保险公司" :before-close="insureDialogClose" :visible.sync="dialogTableVisibleCode" center>
    ...
    <el-dialog :append-to-body="true" width="55%" title="险种" :before-close="riskDialogClose" :visible.sync="dialogTableVisibleCode1" center>
...
import RateUpload from '@/pages/rate_mgr/neiMengRateUpload'
...
export default {
  ...
  methods: {
    ...
    getHandleQuery () {
      this.$refs['queryForm'].validate((valid) => {
        if (valid) {
          ...
          this.Loading()
          this.queryLifeNeimengRate(this.info).then(resp => {
            this.closeLoading()
    ...
    getdbqueryCom () {
      ...
      jsonObj.type = 2
      jsonObj.clickType = 2
      this.getNeimengRateInsuranceCom(jsonObj)
    },
    dbqueryRisk () {
    ...
    getdbqueryRisk () {
      ...
      jsonObj.paymentperiod = this.riskLength
      this.getNeimengRateRisk(jsonObj)
    },
    insureDialogClose () {
    ...
    insuerBlur () {
      if (!(this.dialogTableVisibleCode || this.dialogTableVisibleCode1)) {
        if (this.info.insurerCode === '' && this.info.insurerName === '') {
          this.info.riskCode = ''
          this.info.riskName = ''
          this.riskLength = ''
        } else {
          ...
          api('/ratebranch/getInsures', 'post', temp).then(
            (res) => {
              if (res.data.code !== '000000' && !IsEmpty(res.data.code)) {
                this.info.insurerCode = ''
                this.info.insurerName = ''
              } else {
                this.info.insurerCode = res.data.data.insurercode
                this.info.insurerName = res.data.data.insurername
              }
            }
          )
        }
    ...
    riskBlur () {
      if (!(this.dialogTableVisibleCode || this.dialogTableVisibleCode1)) {
        if (this.info.riskCode === '' && this.info.riskName === '') {
        } else {
          let temp = {
            clickType: 1,
            currentPage: 1,
            ...
            riskCode: this.info.riskCode,
            riskName: this.info.riskName,
            type: '1'
          }
          api('/ratebranch/getRiskCodes', 'post', temp).then(
            (res) => {
              if (res.data.code !== '000000' && !IsEmpty(res.data.code)) {
                this.info.riskCode = ''
                this.info.riskName = ''
                ...
                this.info.riskCode = res.data.data.riskcode
                this.info.riskName = res.data.data.riskname
              }
            }
          )
        }
    ...
    handleRiskRowChange (row, event, column) {
      ...
      this.riskLength = row.paymentperiod
      this.dbRiskSubmit()
    },
    ...
    ...mapActions([
      'queryLifeNeimengRate',
      'getNeimengRateRisk',
      'getNeimengRateInsuranceCom',
      'getBtnLimt_rate'
    ]),
    ...
    dbqueryData (data, op) {
      ...
      if (this.option === 'insurerCode') {
        this.getNeimengRateInsuranceCom(this.info).then(resp => {
          this.list = resp.data
          this.dialogPage.total = resp.totalRecords
        })
        this.diaTitle = '保险公司列表'
      } else if (this.option === 'insurerName') {
        this.getNeimengRateInsuranceCom(this.info)
        this.diaTitle = '保险公司列表'
    ...
    exp () {
      ...
      window.open('/tabycore/ratebranch/exportExcel?insurerCode=' + this.expData.insurerCode + '&riskCode=' + this.expData.riskCode + '&period=' + this.expData.period + '&type=' + this.info.type)
    },
    toCheck () {
      this.$router.push({ path: '/rate/neimengRateCheck' })
    },
