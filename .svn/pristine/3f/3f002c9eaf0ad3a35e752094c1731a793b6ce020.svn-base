  ...
  <div id="riskIndex" >
      <ui-notice-bar slot="breadcast" >
        <el-breadcrumb slot="location" separator=">">
            ...
            <el-breadcrumb-item>续期保单回录</el-breadcrumb-item>
          </el-breadcrumb>
      </ui-notice-bar>
...
export default {
  ...
  methods: {
      ...
      if (this.info.policyNo) {
        this.isBackRecord(this.info).then(resp => {
          ...
          if (resp.data.status === '2') {
            this.msg = `您确定要续期保单回录吗？投保人姓名:${resp.data.message}`
          } else {
    ...
    onBlurPaidPremium (e) {
      ...
      this.info.lifeRenewalPolicyInfoVo.paidPremium = sum.toFixed(2)
    },
    downTem () {
