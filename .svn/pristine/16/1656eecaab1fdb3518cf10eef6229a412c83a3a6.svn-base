<template>
  <div class="marginTB">
    <el-form :rules="lifePartyAConditionListRules" ref="lifePartyAConditionListForm" :model="lifePartyAConditionListForm">
      <ul>
        <li v-for="(item, index) of live" :key="index"  style="list-style-type:none;">
          <el-row>       
            <el-col :span="12">
          ...
          <el-row>       
            ...
            <el-col :span="12">
                <el-form-item  prop="birthdate" >
                  <ui-label-text :required="true"  label="出生日期" labelWidth="120" >
...
export default {
  ...
  mounted () {
    // console.log(JSON.stringify(this.$store.state.proposalForm.lifePartyAConditionList))
    // this.lifePconDist = this.$store.state.proposalForm.lifePartyAConditionList

    this.lifePconDist = this.$store.state.proposalForm.lifePartyAConditionList
    this.dblindex = Number(this.dblindex)
    this.lifePartyAConditionListForm = this.lifePconDist[this.dblindex]
    console.log('生存受益人' + JSON.stringify(this.live))
  ...
  methods: {
    ...
    handleValidClick (index, event) {
      ...
      console.log('修改前   identifyValiddate:' + JSON.stringify(this.live[index].identifyValiddate))
      if (event) {
        this.live[index].longFlag = '1'
        this.live[index].identifyValiddate = ''
      } else {
        this.live[index].longFlag = '0'
        this.live[index].identifyValiddate = ''
        this.open('error', '请选择证件有效期')
      }
      console.log('修改后   longFlag:' + JSON.stringify(this.live[index].longFlag))
      console.log('修改后   identifyValiddate:' + JSON.stringify(this.live[index].identifyValiddate))
    },
    // toast提示
    open (type, mesg) {
      this.$message({
