  ...
  <div class="wrap">
    <ui-notice-bar slot="breadcast" >
      <el-breadcrumb slot="location" separator=">">
        ...
        <el-breadcrumb-item>运营继续率</el-breadcrumb-item>
      </el-breadcrumb>
    </ui-notice-bar>
...
export default {
  ...
  data () {
    return {
      ...
      queryList: [
        ...
        [{label: '地市级分公司机构代码', model: 'comCode', prop: 'comCode', arrId: 3}, {label: '地市级分公司名称', model: 'comName', prop: 'comCName', arrId: 33}]
      ],
      options: [
      ...
      tabelColumList: [
        ...
        {label: '省级分公司代码', prop: 'upComCode'},
        {label: '省级分公司名称', prop: 'upComName'},
        {label: '地级市分公司代码', prop: 'comCode'},
        {label: '地级市分公司名称', prop: 'comName'},
  ...
  methods: {
    getQueryPersistencyOperationReport (currentPage = 1) {
      ...
      api('/persistencyOperationReport/queryPersistencyOperationReport', 'post', info).then(r => {
        console.log(r)
        this.closeLoading()
    ...
    // 导出
    exportData () {
//       api('/persistencyOperationReport/exportPersistencyOperationReport?jixilvFlag=' + this.info.jixilvFlag + '&cRateMonthStart=' + this.info.cRateMonthStart + '&cRateMonthEnd=' + this.info.cRateMonthEnd + '&companyCode=' + this.info.companyCode+'&companyName=' + this.info.companyName + '&upComCode=' + this.info.upComCode + '&upComName=' + this.info.upComName + '&comCode=' + this.info.comCode + '&comName=' + this.info.comName, 'get', this.info).then(r => {
//       })
      window.open(`/tabycore/persistencyOperationReport/exportPersistencyOperationReport?jixilvFlag=` + this.info.jixilvFlag + `&cRateMonthStart=` + this.info.cRateMonthStart + `&cRateMonthEnd=` + this.info.cRateMonthEnd + `&companyCode=` + this.info.companyCode + `&companyName=` + this.info.companyName + `&upComCode=` + this.info.upComCode + `&upComName=` + this.info.upComName + `&comCode=` + this.info.comCode + `&comName=` + this.info.comName)
    },
    // 导出明细
    exportDataDetail (type, row) {
      ...
      window.open(`/tabycore/persistencyOperationReport/downDetails?jixilvFlag=` + info.jixilvFlag + `&cRateMonthStart=` + info.cRateMonthStart + `&cRateMonthEnd=` + info.cRateMonthEnd + `&companyCode=` + info.companyCode + `&companyName=` + info.companyName + `&upComCode=` + info.upComCode + `&upComName=` + info.upComName + `&comCode=` + info.comCode + `&comName=` + info.comName)
//       api('/persistencyOperationReport/downDetails', 'get', info).then(r => {
//       })
    },
    // 保险代码
    ...
    getAllComP (currentPage = 1, clickType = 2) {
      api('/persistency/getAllComP', 'post', {
        clickType,
        currentPage,
        comCodeOrName: this.info.upComCode,
        pageSize: this.dbPage.size
      }).then(r => {
        console.log(r)
        if (r.data.code === '000000') {
    ...
    getAllComC (currentPage = 1, clickType = 2) {
      api('/persistency/getAllComC', 'post', {
        clickType,
        currentPage,
        upperComCode: this.info.upComCode,
        comCodeOrName: this.info.comCode,
        pageSize: this.dbPage.size
      }).then(r => {
        console.log(r)
        if (r.data.code === '000000') {
    ...
    dbqueryCom (e) {
      ...
      if (arrId === '1') {
        ...
        this.getAllComP()
        this.dialogVisible = true
      } else if (arrId === '3' && this.info.upComCode !== '') {
        this.getAllComC()
        this.dialogVisible = true
      } else {
        this.$message({
          message: '请先填写省级分公司机构代码',
          type: 'warning'
        })
      }
    },
    handleBlur (e) {
      ...
      if (this.dialogVisible === false) {
        if (arrId === '1' && this.info.companyCode !== '') {
          ...
          this.getAllComP(1, 1)
        } else if (arrId === '3' && this.info.comCode !== '') {
          this.getAllComC(1, 1)
        }
      }
    },
    ...
    ondownloadDetail (e) {
      ...
      if (e.upComCode.length === '2' || e.upComCode.length === '0') {
        type = 1
      } else if (e.upComCode.length === '6' && e.comCode.length === '0') {
        type = 2
      } else {
        type = 3
      }
